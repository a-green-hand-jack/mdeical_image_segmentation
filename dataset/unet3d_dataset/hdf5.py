...
import augment.unet3d_augment.transforms as transforms
from dataset.unet3d_dataset.utils import (
    get_slice_builder,
    ConfigDataset,
    calculate_stats,
    mirror_pad,
)
from model.unet3d.utils import get_logger
...
class AbstractHDF5Dataset(ConfigDataset):
    ...
    def __init__(
        ...
        with h5py.File(file_path, "r") as f:
            ...
            weight_map = (
                f[weight_internal_path] if weight_internal_path is not None else None
            )
            # build slice indices for raw and label data sets
            slice_builder = get_slice_builder(
                raw, label, weight_map, slice_builder_config
            )
        ...
        if self.phase == "test":
            ...
            if self.weight_internal_path is not None:
                ...
                weight_patch_transformed = self.weight_transform(
                    self.get_weight_patch(weight_idx)
                )
                return (
    ...
    def _check_volume_sizes(self):
        ...
        with h5py.File(self.file_path, "r") as f:
            ...
            if self.weight_internal_path is not None:
                ...
                assert weight_map.ndim in [
                    3,
                    4,
                ], "Weight map dataset must be 3D (DxHxW) or 4D (CxDxHxW)"
                assert _volume_shape(raw) == _volume_shape(
                    weight_map
                ), "Raw and weight map have to be of the same size"
    ...
    def get_raw_padded_patch(self, idx):
        if self._raw_padded is None:
            with h5py.File(self.file_path, "r") as f:
                ...
                self._raw_padded = mirror_pad(
                    f[self.raw_internal_path][:], self.halo_shape
                )
